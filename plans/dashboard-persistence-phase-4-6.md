...
**Status: Phases 4-5 ✅ COMPLETE, Phase 6 ⏳ NOT STARTED** (2026-01-25)
...
### Deliverables
- [x] Provisioner uses discovery module
- [x] DEFAULT_DASHBOARDS removed (replaced with get_default_dashboards() for backwards compat)
- [x] Extension filtering works

### Implementation Notes
- `provision_all(extension=None)` now discovers dashboards automatically
- `delete_all(extension=None)` uses discovery for dashboard list
- `_ensure_folder()` now handles per-extension folders with folder_uid
- `list_provisioned(extension=None)` filters by extension
- Backwards compatibility via `get_default_dashboards()` function
...
### Deliverables
- [x] `contextcore dashboards provision -e squirrel` works
- [x] `contextcore dashboards list --source local` works
- [x] `contextcore dashboards extensions` shows all extensions

### Implementation Notes
- Added `grafana_options` decorator for common Grafana connection options
- Extension validation with helpful error messages
- `--source` flag supports: local, grafana, both
- Output grouped by extension with folder paths
...
1. ✅ `contextcore dashboards provision` provisions all 11 dashboards automatically
2. ✅ `contextcore dashboards provision -e core` only provisions 5 core dashboards
3. ✅ `contextcore dashboards extensions` lists 8 extensions with dashboard counts
